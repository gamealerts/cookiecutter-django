...
If you are using `docker-toolbox` to develop on OS X or Windows, you need to create a `/data` partition inside the
virtual machine that runs docker to make all changes persistent. If you don't do that your `/data` directory will get wiped out on every reboot.
...
In case you are wondering why you can't use a host volume to keep the files on your mac: As of `docker-toolbox` 1.8 you'll
